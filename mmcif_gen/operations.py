...
class StaticValueOperation(operationBase):
    def perform_operation(self, operation_data):
        ...
        data = self.investigation_storage.data[target_category]
        first_item = next(iter( self.investigation_storage.data[target_category]), None)
        rows_to_write = max(len(self.investigation_storage.data[target_category].get(first_item,[])), 1)
